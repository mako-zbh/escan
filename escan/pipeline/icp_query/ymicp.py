...
def is_public_ipv6(ipv6):
    """检查 IPv6 是否为公网地址：排除回环、链路本地、ULA、文档前缀。"""
    # ::1 回环, fe80::/10 链路本地, fc00::/7 ULA, 2001:db8::/32 文档
    return not (
        ipv6 == "::1"
        or ipv6.startswith("fe80")
        or ipv6.startswith("fc")
        or ipv6.startswith("fd")
        or ipv6.startswith("2001:db8")
    )
...
def get_local_ipv6_addresses():
    ...
    try:
        if os.name == 'nt':  # Windows
            ...
            # 先尝试 Linux ip 命令，失败则 fallback 到 macOS ifconfig
            ...
            if output:
                ...
                # macOS: 使用 ifconfig
                output = _run_cmd_capture(["ifconfig"])
                if output:
                    current_inet6 = None
                    for line in output.splitlines():
                        stripped = line.strip()
